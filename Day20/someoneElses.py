	...
	for variation in generate_matrix_variations(full_image):
		...
		if len(monsters) > len(most_monsters):
			...
			for row in variation:
				roughest += row.count('#')
	return roughest, most_monsters, full_image


f = open('input.txt')
raw = f.read()

roughest, most_monsters, full_image = solve_two(raw)
print(roughest, most_monsters)
#full_image = [''.join(row) for row in full_image]
#print(*full_image, sep = '\n')
